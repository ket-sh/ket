...
function switchFor(group: string | undefined): HTMLElement | null {
  return document.querySelector<HTMLElement>(
    `.audience-switch[data-audience-group="${group ?? ''}"]`,
  );
}

function offered(buttons: HTMLElement | null, wanted: string): boolean {
  if (buttons === null) {
    return false;
  }

  const option = buttons.querySelector<HTMLButtonElement>(`[data-audience="${wanted}"]`);

  return option !== null && !option.disabled;
}

function paintGroup(variants: HTMLElement, wanted: string): void {
...
function showAudience(wanted: string): void {
...
export function wireAudience(): void {
  showAudience(localStorage.getItem(AUDIENCE_STORE) ?? 'technical');
